...
//       from: process.env.SMTP_FROM_MAIL || "oveniacompany@gmail.com",
//       to: userEmail,
...
const orderStatusEmailContent = (userName, orderId, status) => `
  <div style="font-family: Arial, sans-serif; line-height: 1.6;">
    <h2>Order ${status}</h2>
    <p>Hi ${userName},</p>
    <p>Your order <b>${orderId}</b> has been <b>${status}</b>.</p>
    <p>Thank you for shopping with Ovenia.</p>
  </div>
`;
...
const shippingStatusEmailTemplates = {
  dispatched: (userName, orderId) => `
    <h2>Order Dispatched</h2>
    <p>Hi ${userName}, your order <b>${orderId}</b> has been dispatched.</p>
  `,
  outForDelivery: (userName, orderId) => `
    <h2>Out for Delivery</h2>
    <p>Hi ${userName}, your order <b>${orderId}</b> is out for delivery.</p>
  `,
  delivered: (userName, orderId) => `
    <h2>Order Delivered</h2>
    <p>Hi ${userName}, your order <b>${orderId}</b> has been delivered.</p>
  `,
};
...
async function sendOrderStatusEmail(userEmail, userName, orderId, status) {
  const html =
    shippingStatusEmailTemplates[status]?.(userName, orderId) ||
    orderStatusEmailContent(userName, orderId, status);
